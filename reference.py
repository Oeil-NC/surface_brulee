...
def creation_couche_fusionnee(datedate_field, fcSentinel, fcDSCGR, fcVIIRS, fcGS,\
    Nom_sortie, where_clause, where_clause_ref, buffer):
    ...
    fields = [field.name for field in arcpy.ListFields(arcpy.env.workspace + os.sep + Nom_sortie) if
            field.name not in ["Num_S", "Nature", datedate_field[0], datedate_field[1], "count_overlaps", "OBJECTID", "Shape", "Shape_Length", "Shape_Area","OBJECTID_1", "OBJECTID_12"]]
    for f in fields:
        arcpy.DeleteField_management(arcpy.env.workspace + os.sep + Nom_sortie, f)
        arcpy.DeleteField_management(arcpy.env.workspace + os.sep + Nom_sortie + "_Buffer", f)
    arcpy.RepairGeometry_management(arcpy.env.workspace + os.sep + Nom_sortie)
    arcpy.RepairGeometry_management(arcpy.env.workspace + os.sep + Nom_sortie + "_Buffer")
